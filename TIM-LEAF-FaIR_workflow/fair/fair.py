...
#
        ...
        df_conc = pd.read_csv('data/rcmip-concentrations-annual-means-v5-1-0.csv')
        df_forc = pd.read_csv('data/rcmip-radiative-forcing-annual-means-v5-1-0.csv')
        ...
        for scenario in self.scenarios:
            for specie, specie_rcmip_name in species_to_rcmip.items():
                if self.properties_df.loc[specie, "input_mode"] == "emissions":
                    ...
                    emis_in = (
                        df_emis.loc[
                            (df_emis["Scenario"] == RCMIPscenario)
                            & (
                                df_emis["Variable"].str.endswith(
                                    "|" + specie_rcmip_name
                                )
                ...
                if self.properties_df.loc[specie, "input_mode"] == "concentration":
                    ...
                    conc_in = (
                        df_conc.loc[
                            (df_conc["Scenario"] == RCMIPscenario)
                            & (
                                df_conc["Variable"].str.endswith(
                                    "|" + specie_rcmip_name
                                )
                ...
                if self.properties_df.loc[specie, "input_mode"] == "forcing":
                    ...
                    forc_in = (
                        df_forc.loc[
                            (df_forc["Scenario"] == RCMIPscenario)
                            & (
                                df_forc["Variable"].str.endswith(
                                    "|" + specie_rcmip_name
                                )
